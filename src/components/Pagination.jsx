...
const Pagination = ({queries}) => {
  ...
  return (
    <div className="flex flex-col items-center max-w-[700px] py-14">
      <div className="relative text-[#4285f4]">
        ...
        <img className="w-[250px] md:w-[300px]" src={Logo} alt="gooogle_img" />
        {queries.nextPage && (
          <div
      ...
      <div className="flex gap-3 text-[#4285f4] text-sm">
        {pagination.map((p) => (
            <span className = {`cursor-pointer ${page === p.startIndex ? "text-black" : ""}`} key = {p.page} onClick={() => {paginationClickHandler(p.startIndex)}}>{p.page}</span>
        ))}
      </div>
    </div>
  );
